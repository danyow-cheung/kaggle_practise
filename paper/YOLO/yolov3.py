...
def load_weights(var_list,weights_file):
	...
	while i<len(var_list):
		...
		var2 = var_list[i+1]
		# 2.4 to be continue 
		# doing something only if we process conv layer 
		if 'Conv' in var1.name.split('/')[-2]:
			...
			if 'BatchNorm' in var2.name.split('/')[-2]:
				...
				for var in batch_norm_vars:
					...
					ptr+= num_params
					assign_ops.append(tf.assign(var,var_weights,validate_type=True))
				...
				bias_weights  = weights[bias_shape]
				ptr += bias_params
				assign_ops.append(tf.assign(bias,bias_weights,validate_type=True))
			...
			ptr+=num_params
			assign_ops.append(tf.assign(var1,var_weights,validate_type=True))
			i+=1 
	return assign_ops 
...
def non_max_supreesion(predictions_with_boxes,confidence_threshold,iou_threshold=0.4):
	'''applies non-max supression to prediction boxed
	:params predictions_with_boxes:	3D numpy array first 4 values in3rd dimension are bbox attrs,5th is confidence
	:params confidence_threshold:the threshold for deciding if prediction is valid 
	:params iou_threshold :the threshold for detecting if prediction is valid 

	:params dict:the threshold for detecting if two boxes overlap 
	:return dict:class->[(box,score)]	
	'''

	conf_mask = np.expand_dims((predictions_with_boxes[:,:,4]>confidence_threshold),-1)
	confidence = predictions_with_boxes*conf_mask
	...
	for i,image_pred in enumerate(predictions):
		...
		for cls in unique_classes:
			...
			cls_boxes = bbox_attrs[np.nonzero(image_pred)]
			...
			while len(cls_boxes)>0:
				...
				if not cls in result:
					resutlt[cls] = []
				resutlt[cls].append((box,score))
				cls_boxes = cls_boxes[1:]
				ious = np.array(_iou(box,x) for x in cls_boxes)
				iou_mask = iouts<iou_threshold
