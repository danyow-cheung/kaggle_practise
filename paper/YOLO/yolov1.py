...
'''Define the model as bescribed in the original paper'''
...
model.add(Conv2D(filtes=192,
				kernel_size=(3,3),
				padding='same',
				activation=lrelu,
				kernel_regularizier = l2(5e-4)))

model.add(MaxPooling2D(pool_size=(2,2),strides=(2,2),padding='same'))

model.add(Conv2D(filtes=128,
				kernel_size=(1,1),
				padding='same',
				activation=lrelu,
				kernel_regularizier = l2(5e-4)))

model.add(Conv2D(filtes=256,
				kernel_size=(3,3),
				padding='same',
				activation=lrelu,
				kernel_regularizier = l2(5e-4)))
...
class CustomLearningRateScheduler(keras.callbacks.Callback):
	...
	Arguments:
		schedule: a function that takes an epoch index (integer,indexed from 0) and current learning rate
				  as a inputs and returns a new learning rate as output(float)
		
	'''

	def __init__(self,schedule):
		super(CustomLearningRateScheduler).__init___()	
...
def yolo_head(feats):
	# Dynamic inplementation of conv dims for fully convolutional model
